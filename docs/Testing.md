...
Память угла последнего выстрела отдельно проверяется на живой сцене игрока:

```sh
godot --headless --path . --script tests/aiming_memory_runtime_check.gd
```

Скрипт держит кнопку действия, стреляет из лука под углом, убеждается, что
стояние на полу не стирает память, что второе прицеливание повторяет угол,
что ходьба память стирает и что окно длится свои 2 секунды. Проверка ждёт по
реальному времени, поэтому `--max-fps` не обязателен.
