...
- **Делает:** Управляет двумя наборами рук и носимыми слотами, требованиями, обменом, боеприпасами и разрешёнными действиями.
- **Когда применять:** Когда предметы должны давать урон, защиту и доступные действия.
- **Что требуется:** Для операций с вещами — InventoryComponent; CharacterAttributes добавляет требования и расчёт нагрузки. Ввод не обязателен.
...
- **Делает:** Хранит силу, ловкость, интеллект, выносливость и мудрость; проверяет требования предметов и считает производные значения.
- **Когда применять:** Для требований экипировки, максимума здоровья, маны и нагрузки.
...
- **Настройки и ограничения:** Damage Per Tick и Tick Interval находятся в ресурсе эффекта; сопротивления 0–100% — в Dot Resistances → Resistance на компоненте цели. Список типов берётся из общего DotCatalog и расширяется генератором. Первый тик — через интервал, не сразу. Для одного ID сильнейший по Damage Per Tick DOT заменяет слабый с новой длительностью и отсчётом тика; слабый не заменяет и не продлевает сильный. Равный обновляет длительность и при прежнем интервале сохраняет ближайший тик. Стаков нет. При 100% сопротивления эффект остаётся, но урона нет. Компонент не умножает характеристики; Damage Per Tick = 0 оставляет обычный временный статус.
...
| [ItemStats](../features/inventory/ItemStats.gd) | Урон, защита и требования к базовым характеристикам; наличие дополнительных полей не означает готовую механику |
