...
- Общий размер: корень `Enemy` / `FlyingEnemy` → Transform → Scale,
  одинаковые X и Y. Исходные значения: `0.24` и `0.22` соответственно.
  ...
  вместе с корнем. Например, для удвоения наземного врага задайте `0.48, 0.48`.
