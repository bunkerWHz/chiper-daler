...
func test_endurance_changes_max_health_without_resurrecting() -> void:
	...
	assert_eq(health.get_max_health(), 106.0)
	assert_eq(health.get_current_health(), 106.0)
	health.take_damage(30.0)
	attributes.set_endurance(8)
	assert_eq(health.get_max_health(), 107.0)
	assert_eq(health.get_current_health(), 77.0)
