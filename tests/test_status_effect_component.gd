...
func test_buff_and_debuff_overlap_refresh_and_expire() -> void:
	...
	state.refresh_state()
	assert_true(state.has_condition(ActorState.Condition.BUFFED))
	assert_true(state.has_condition(ActorState.Condition.DEBUFFED))
	...
	state.refresh_state()
	assert_true(state.has_condition(ActorState.Condition.BUFFED))
	assert_false(state.has_condition(ActorState.Condition.DEBUFFED))

	effects._process(0.25)
	state.refresh_state()
	assert_false(state.has_condition(ActorState.Condition.BUFFED))
