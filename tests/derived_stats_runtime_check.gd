...
func _run() -> void:
	...
	regen._process(0.1)
	near(health.get_current_health(), 21, "Combat HP tick")
	near(mana.get_mana(), 21, "Combat mana tick")
	near(stamina.get_stamina(), 21, "Combat stamina tick")
	regen._process(3.0)
	near(health.get_current_health(), 54, "Three more combat ticks")
	regen._process(1.0)
	near(health.get_current_health(), 76, "Fifth second enables double regeneration")
	...
	regen._process(1.0)
	near(health.get_current_health(), 87, "Attack restores combat regeneration rate")
	...
	health.take_damage(77)
	mana._set_mana(10)
	stamina.spend(stamina.get_stamina() - 10)
	regen._process(1.0)
	near(health.get_current_health(), 24, "Equipment adds HP regeneration")
	near(mana.get_mana(), 25, "Equipment adds mana regeneration")
	near(stamina.get_stamina(), 26, "Equipment adds stamina regeneration")
	equipment.unequip_item(ItemData.EquipSlot.CHEST)
	regen._process(1.0)
	near(health.get_current_health(), 35, "Unequipping removes regeneration bonus")
