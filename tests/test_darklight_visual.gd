...
func test_active_equipment_updates_both_skeletal_hand_attachments() -> void:
	...
	assert_true(off.has_node("ItemVisual"))
	assert_true(equipment.switch_weapon_set(1))
	assert_false(main.visible)
	assert_false(main.has_node("ItemVisual"))
	...
	assert_false(off.has_node("ItemVisual"))
...
func test_fsm_selects_attack_variants_and_facing_preserves_physics() -> void:
	...
	var expected := {
		...
		ActorState.Behavior.DODGE: &"dodge_roll",
		ActorState.Behavior.GROUND_LIGHT_ATTACK: &"attack",
		ActorState.Behavior.GROUND_HEAVY_ATTACK: &"heavy_attack",
		ActorState.Behavior.AIR_LIGHT_ATTACK: &"air_attack",
...
func test_dodge_state_picks_the_air_clip_or_the_ground_roll() -> void:
	var setup := _create_player_visual()
	var visual := setup.visual as DarklightVisualComponent
	var player := setup.player as Actor
	var state := player.get_component(ActorStateComponent) as ActorStateComponent
	...
	var animation := visual.get_animation_player()
	assert_true(animation.has_animation(&"dodge_air"))
	assert_true(animation.has_animation(&"dodge_roll"))
	# DodgeComponent decides the mode when the dodge starts; the visual only reads it.
	for entry: Array in [[false, &"dodge_roll"], [true, &"dodge_air"]]:
		dodge._is_air_dodge = bool(entry[0])
		state.state_changed.emit(ActorState.Behavior.FALL, ActorState.Behavior.DODGE)
		assert_eq(animation.current_animation, entry[1])
		state.state_changed.emit(ActorState.Behavior.DODGE, ActorState.Behavior.FALL)
	# Each mode is retimed against its own configured duration.
	for entry: Array in [[false, &"dodge_roll", dodge.config.roll_duration], [true, &"dodge_air", dodge.config.duration]]:
		dodge._is_air_dodge = bool(entry[0])
		state.state_changed.emit(ActorState.Behavior.FALL, ActorState.Behavior.DODGE)
		assert_true(is_equal_approx(
			animation.get_playing_speed(),
			animation.get_animation(entry[1]).length / float(entry[2])
		))
		state.state_changed.emit(ActorState.Behavior.DODGE, ActorState.Behavior.FALL)
...
func test_dodge_roll_is_an_authored_tucked_roll() -> void:
	# The roll is generated by tests/dodge_roll_author.gd; this guards the
	# shape that the gameplay-facing clip must keep.
	...
	assert_true(player.has_animation(&"dodge_roll"))
	var clip := player.get_animation(&"dodge_roll")
	...
	for index in clip.get_track_count():
		var keys := clip.track_get_key_count(index)
		assert_true(keys > 1)
		assert_true(clip.track_get_key_time(index, 0) >= 0.0)
		assert_true(clip.track_get_key_time(index, keys - 1) <= clip.length + 0.00001)
	...
	assert_true(is_zero_approx(float(clip.track_get_key_value(rotation_track, 0))))
	assert_true(is_equal_approx(float(clip.track_get_key_value(rotation_track, last)), TAU))
	var start := clip.track_get_key_value(position_track, 0) as Vector2
	var finish := clip.track_get_key_value(position_track, last) as Vector2
	assert_true(start.is_equal_approx(finish))
	assert_true(absf(start.y - 136.0) < 12.0)
	# A tucked tumble has to lift the pelvis: the body rolls over the head sweep.
	var highest := INF
	...
	for index in mini(clip.track_get_key_count(leg_track), clip.track_get_key_count(position_track)):
		...
		var pelvis := clip.track_get_key_value(position_track, index) as Vector2
		closest = minf(closest, leg.distance_to(pelvis))
	assert_true(closest < 200.0, "The foot must tuck against the pelvis mid-roll")
