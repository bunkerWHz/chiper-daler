...
Depth-order update: the near arm (`arm_right`) now sits at X = -0.42, between torso (X ≈ -0.572) and head (X = -0.3). Both the Idle and Movement working scenes and their sprite exports have this correction; the original desktop source artwork remains untouched.
