...
func _ready() -> void:
	...
	_detail_popup.configure(_inventory, _equipment, _flask_charges)
...
func _rebuild_grid() -> void:
	...
	if _selected_category == ALL_CATEGORIES:
		for _empty_index in _inventory.get_capacity() - stacks.size():
			var empty_cell := ITEM_CELL_SCENE.instantiate() as InventoryDragButton
...
func _rebuild_details() -> void:
	...
	_detail_popup.set_text(_detail_popup.get_item_text(item))
	if _details_pinned:
...
func show_item_details(item_id: StringName) -> void:
	...
	if item == null:
		return
	_show_details_text(_detail_popup.get_item_text(item))
...
func hide_hover_details() -> void:
	if _details_pinned:
		...
		if selected != null:
			_detail_popup.set_text(_detail_popup.get_item_text(selected))
